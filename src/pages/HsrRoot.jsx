...
function HsrRoot() {
    ...
    getData(
      ...
      function (res) {
        ...
        for (var i = 0; i < HSR_Station_Data.length; i++) {
          temparr.push(
            `${HSR_Station_Data[i].StationName.Zh_tw}(${HSR_Station_Data[i].StationID})`
          );
        }
  ...
  React.useEffect(() => {
    if (UrlParam("sw")) {
      setRadioValue(UrlParam("sw"));
    } else {
      setRadioValue("station");
    }
      ...
      <Box sx={{ p: 3 }}>
        ...
        <div hidden={radioValue === "train"}>
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField {...params} label="車站名稱" ref={stationInput} />
            )}
            noOptionsText="無資料"
          />
          <p></p>
          <Alert severity="warning">
            注意: 區分 "台" 和 "臺"! 高鐵資料使用的是"台"
          </Alert>
        </div>
        ...
        <Button
          ...
          to={
            ...
            radioValue +
            "/?q=" +
            (radioValue === "station" ? selected : trainNum) +
            "&t=fulldata"
          }
        >
          繼續
